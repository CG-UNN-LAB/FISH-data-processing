...
# Основная фун-я по сегментации:
    def predict_image(self):                     # ИСПРАВИТЬ: не допускать точности, ниже 0.2;
        if Func.FILEPATH != '-1':       # Значит, что картинка была сохранена, путь есть;
            ...
            folder_path = '..\\Photo_Qt\\' + Func.FILENAME  # Имя папки. If ниже проверяет, существует ли такая папка, есла да - удалить.
            if os.path.exists(folder_path):                          # Фун-я model.predict не может сама перезаписать папку с полученным изображением;
                shutil.rmtree(folder_path)

            predictions = Func.model.predict(img, 
                                        ...
                                        line_thickness=1) # Толщина границ выделения;
            Path = '..\\Photo_Qt\\' + Func.FILENAME + '\\' + Func.FILENAME + '.jpg' # Берем путь, чтобы вывести на экран;
            pixmap = QPixmap(Path)
            ...
            print("Whole:")
            print(number_whole)
            ref = "Whole cell: " + str(number_whole) + "\nExplode cell: " + str(number_explode)
...
# фун-я, сохраняющая картинку:
    def PhotoSave(self, pixmap, filename):
        ...
        Image_name = os.path.splitext(os.path.basename(filename))[0] # Для получения имени файла без расширения и пути к нему;

        dir_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "..\\Photo_Qt")
        jpg = '.jpg'
        # print (dir_path)
        filepath = os.path.join(dir_path, Image_name) # Объединяет путь к директории (dir_path) и имя файла (Image_name), чтобы создать полный путь к файлу;
