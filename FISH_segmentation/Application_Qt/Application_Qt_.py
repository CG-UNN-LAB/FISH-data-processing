    ...
    def predict_image(self): # ИСПРАВИТЬ: не допускать точности, ниже 0.2;
        if Func.FILEPATH != '-1':                # Значит, что картинка была сохранена, путь есть;
            ...
            predictions = Func.model.predict(img, 
                                        ...
                                        #name = "photos_and_labels",     # model.predict сначала создасть папку с именем Ui_MainWindow.FILENAME (внутри папки project = ' '), только затем положит туда изображнеие с таким же названием, как и img; 
                                        show_labels = False, 
                                        show_conf = False,
                                        save_txt = False,     # Сохранит рядом с обработанным изображением в папку Ui_MainWindow.FILENAME;
